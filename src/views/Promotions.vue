<template>
  <div class="promotions-page">
    ...
    <!-- 📝 Form thêm / sửa -->
    <form class="promotion-form" @submit.prevent="savePromotion">
      ...
      <div class="form-group">
        ...
        <input v-model="promotion.promo_code" type="text" required placeholder="Nhập mã khuyến mãi" />
      </div>

      <div class="form-group">
        <label>Mô tả</label>
        <input v-model="promotion.description" type="text" placeholder="Mô tả khuyến mãi" />
      </div>

      <div class="form-group">
        <label>Loại giảm giá</label>
        <select v-model="promotion.discount_type" required>
      ...
      <div class="form-group">
        ...
        <input v-model="promotion.discount_value" type="number" step="0.01" required />
      </div>

      <div class="form-group">
        <label>Ngày bắt đầu</label>
        <Flatpickr v-model="promotion.start_date" :config="{ dateFormat: 'Y-m-d' }" placeholder="Chọn ngày bắt đầu" />
      </div>

      <div class="form-group">
        <label>Ngày kết thúc</label>
        <Flatpickr v-model="promotion.end_date" :config="{ dateFormat: 'Y-m-d' }" placeholder="Chọn ngày kết thúc" />
      </div>

      <div class="form-group">
        <label>Giá trị tối thiểu đơn hàng</label>
        <input v-model="promotion.min_order_amount" type="number" step="0.01" />
      </div>

      <div class="form-group">
        <label>Giới hạn sử dụng</label>
        <input v-model="promotion.usage_limit" type="number" min="0" />
      </div>
    ...
    <table class="promotion-table">
      ...
      <tbody>
        <tr v-for="p in filteredPromotions" :key="p.promo_id">
          ...
          <td>{{ p.status }}</td>
          <td>
            <button @click="editPromotion(p)">✏️</button>
            <button @click="deletePromotion(p.promo_id)">🗑️</button>
          </td>
        </tr>
